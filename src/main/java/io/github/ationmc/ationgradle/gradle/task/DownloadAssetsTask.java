...
import io.github.ationmc.ationgradle.gradle.util.DownloadUtil;
...
public class DownloadAssetsTask extends Task {
    ...
    public void downloadAssets() {
        String asset = ClientUtil.getClientAsset(extension);
        File index = ClientUtil.getClientIndexFile(extension);

        //download index
        try {
            if (!ClientUtil.fileVerify(index, ClientUtil.getClientAssetSha1(extension))) {
        ...
        objectHashMap.forEach((name, assetObject) -> {
            File local = ClientUtil.getLocalClientObjectFile(assetObject.getHash());
            File objectFile = ClientUtil.getClientObjectFile(extension, assetObject.getHash());

            try {
                if (!ClientUtil.fileVerify(objectFile, assetObject.getHash())) {
                    if (ClientUtil.fileVerify(local, assetObject.getHash())) {
                        try {
        ...
        //download native
        File nativeJarDir = ClientUtil.getNativeJarDir(extension);
        File nativeFileDir = ClientUtil.getNativeFileDir(extension);

        ClientUtil.getNatives(extension).forEach(link -> {
