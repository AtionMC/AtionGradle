 ...
 * @author Enaium
 */
public class GenIntelliJRunsTask extends Task {
    @TaskAction
    public void generateIdeaRun() {
        StringBuilder vmArgs = new StringBuilder("-Djava.library.path=" + ClientUtil.getNativeFileDir(extension).getAbsolutePath());
        ...
        programArgs.append("--assetsDir").append(" ").append(ClientUtil.getClientAssetDir(extension)).append(" ");
        programArgs.append("--assetIndex").append(" ").append(extension.minecraft.version).append(" ");
        programArgs.append("--version").append(" ").append("ationmc").append(" ");
        // programArgs.append("--version").append(" ").append("humble").append(" ");
        programArgs.append("--accessToken").append(" ").append("0").append(" ");

        try {
            String idea = IOUtils.toString(Objects.requireNonNull(GenIntelliJRunsTask.class.getResourceAsStream("/IDEA_RUN_CONFIGURATION.xml")), StandardCharsets.UTF_8);
            idea = idea.replace("%NAME%", "AtionMC Client Run");
            ...
            File runConfigs = new File(runConfigsDir, "AtionMC_Client_Run" + projectPath + ".xml");
