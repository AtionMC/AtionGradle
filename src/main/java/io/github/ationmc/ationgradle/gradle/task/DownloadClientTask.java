 ...
 * @author Enaium
 */
public class DownloadClientTask extends Task {
    @TaskAction
    public void download() {
        File clientJar = ClientUtil.getClientFile(extension);
        try {
            if (!ClientUtil.fileVerify(clientJar, ClientUtil.getClientJarSha1(extension))) {
                File localJar = ClientUtil.getLocalJar(extension.minecraft.version);
                if (ClientUtil.fileVerify(localJar, ClientUtil.getClientJarSha1(extension))) {
                    ...
                    FileUtils.writeByteArrayToFile(clientJar, ClientUtil.getClientJar(extension));
                }
            }
        } catch (IOException e) {
