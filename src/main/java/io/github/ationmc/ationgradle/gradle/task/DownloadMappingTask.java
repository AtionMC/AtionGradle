...
public class DownloadMappingTask extends Task {
    ...
    public void downloadMapping() {
        File clientMappingFile = ClientUtil.getClientMappingFile(extension);
        try {
            if (!ClientUtil.fileVerify(clientMappingFile, ClientUtil.getClientMappingSha1(extension))) {
                getProject().getLogger().lifecycle("Download client.txt");
                FileUtils.write(clientMappingFile, ClientUtil.getClientMapping(extension), StandardCharsets.UTF_8);
            }
        } catch (IOException e) {
